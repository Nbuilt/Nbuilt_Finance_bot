# -*- coding: utf-8 -*-
print("🔥 FINANCE BOT FINAL | PHONE + LINK SCHEME 🔥")

import asyncio, sqlite3, secrets, datetime
...
SUPER_ADMINS = [5378186366]  # admin telegram ID(lar)
...
# ================== XOTIRA ==================
waiting_phone = set()          # admin telefon kiritish rejimi
invite_tokens = {}             # token -> phone
selected_client = {}           # admin_id -> phone
pending_payments = {}          # pid -> (phone, amount, admin_id)
...
async def start(message: types.Message):
    ...
    # 2️⃣ TELEGRAM ID OLDINDAN BOR MIJOZ
    cur.execute(
    ...
    await message.answer(
        "⛔ Sizda ruxsat yo‘q.\n"
        "Admin bergan havola orqali kiring."
    )

# ================== MIJOZ QO‘SHISH (TELEFON) ==================
...
async def save_phone(m: types.Message):
    ...
    await m.answer(
        "✅ Mijoz qo‘shildi.\n\n"
        f"📞 Telefon: {phone}\n\n"
        "Agar Telegram akkaunti bo‘lmasa, shu havolani yuboring:\n"
        f"{link}"
    )
